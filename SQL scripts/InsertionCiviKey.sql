...
insert into dbo.tContact(Name,Description,LogoPath)values('Fondation Steria','<p>La Fondation Steria – Institut de France s’inscrit dans la politique de solidarité du Groupe Steria, qui a pour objectif de favoriser l’autonomie de publics en difficulté en leur donnant accès à l’éducation, à l’informatique et à un métier. La Fondation est née d’une démarche participative de collaborateurs de Steria en France. Elle a pour objectif de contribuer à la réduction de la fracture numérique.</p>',''); --6
insert into dbo.tContact(Name,Description,LogoPath)values('Alcatel-Lucent','Société Alcatel-Lucent',''); --7
insert into dbo.tContact(Name,Description,LogoPath)values('IN''TECH INFO','<p>Différents groupes d''élèves se sont succédés à IN''TECH INFO pour travailler sur le Civikey depuis sa création. L''école a toujours voulu accompagner et faire vivre ce projet à vocation humaine. Il est une magnifique vitrine de leur savoir faire et des compétences mises en oeuvre. Ce travail d''équipe entre différentes entités correspond parfaitement à la pédagogie de projets développée à IN''TECH INFO qui souhaite continuer à collaborer à la réussite de ce projet encore de nombreuses années.</p>',''); --8
